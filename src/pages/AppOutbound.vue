<template>
  <div class="app-outbound">
    ...
    <div class="outbound_search mt-10">
      <Input search placeholder="请输入租户名称" style="width:458px;" @on-search="queryList" />
    </div>
    <div class="add mt-25">
      ...
      >
        <FormItem label="学校名称" prop="name">
          <Select
            ...
            placeholder="请选择..."
          >
            <Option v-for="(option, index) in schoolList" :value="option.value" :key="index">
              {{ option.label }}
            </Option>
          </Select>
        </FormItem>
        <FormItem label="学校代码" prop="code">
          <Input v-model="formCreate.code" placeholder="选择学校后显示"></Input>
        </FormItem>
      ...
      columns1: [
        ...
        },
        {
          ...
          render: (h, row) => {
            return (
              <div>
                <span class="split" onClick={() => this.showOutbound()}>
                  应用出库
                </span>
